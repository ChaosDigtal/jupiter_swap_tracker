...
async function getUSDPriceForTokens(tokens: string) {
    try {
        let payload = (await got
            .get(`https://lite-api.jup.ag/price/v2?ids=${tokens},So11111111111111111111111111111111111111112`)
            .json()) as any;
        ...
        for (const token in tokenUSDPrice) {
            if (tokenUSDPrice.hasOwnProperty(token)) {
                if (tokenUSDPrice[token] == null)
        ...
        try {
            tokenUSDPrice = (await getSolTokenPrice(tokens)) as PriceData
